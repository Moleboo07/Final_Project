...
#chargement des différents csv, remplacer le chemin d'accés
df = pd.read_csv("C:\\Users\\poire\\OneDrive\\Bureau\\topic_pe\\pour_concatener.csv", sep=";")
df_2 = pd.read_csv("C:\\Users\\poire\\OneDrive\\Bureau\\topic_pe\\channel_2.csv", sep=";")
df_3 = 	pd.read_csv("C:\\Users\\poire\\OneDrive\\Bureau\\topic_pe\\can_neg.csv", sep=";")


#a utiliser si npous voulons faire l'analyse globale 
#concatenated_df = pd.concat([df, df_2], ignore_index=True)


#ne mettre que le nom du data frame que nous souhaitons analyser (si analyse séparée)
concatenated_df = df_3
def bertopic_fct(df_APE_DL):

	#mettre les commentaires sans majuscules, sans accent, sans ponctuation
	...
	topic_model = BERTopic(language="french", min_topic_size=20, nr_topics=10, top_n_words= 10)
	...
	fig, axs = plt.subplots(len(data['data']), 1, figsize=(5, len(data['data']) * 3), sharex=True)
	...
	#plt.show()
	df_APE_DL.to_csv("C:\\Users\\poire\\OneDrive\\Bureau\\topic_pe\\topic_glo_sp.csv", encoding='utf-8', sep=';', index=False)

bertopic_fct(concatenated_df)
